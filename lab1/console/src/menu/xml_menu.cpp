...
xml_menu::xml_menu(menu_sptr parent) : base_menu({
  "1. Create xml file"sv,
  "2. Serialize object to xml"sv,
  "3. Read xml file"sv,
  "4. Remove file"sv,
  "8. Previous menu"sv,
  "0. Exit"sv
...
menu_sptr xml_menu::process() {
  auto self = shared_from_this();
  menu_sptr next_menu = self;

  switch (choose()) {
    case '1': {
      filemanager::create_file(get_filename() + ".xml");
    } break;

    case '2': {
      text_format::from(get_some_pod()).save_as_xml(get_filename() + ".xml");
    } break;

    case '3': {
      std::cout << filemanager::read_all(get_filename() + ".xml");
    } break;

    case '4': {
      filemanager::delete_file(get_filename() + ".xml");
    } break;
...
some_pod xml_menu::get_some_pod() const {
  some_pod result;
